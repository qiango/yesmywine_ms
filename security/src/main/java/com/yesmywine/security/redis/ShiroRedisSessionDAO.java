...
public class ShiroRedisSessionDAO extends EnterpriseCacheSessionDAO {
    ...
    private void saveSession(Session session) throws UnknownSessionException{
        ...
        byte[] value = ShiroSerializeUtils.serialize(session);
    ...
    public Collection<Session> getActiveSessions() {
        ...
        if(keys != null && keys.size()>0){
            for(byte[] key:keys){
                Session s = (Session) ShiroSerializeUtils.deserialize(redisManager.get(key));
                sessions.add(s);
            }
        }
    ...
    protected Session doReadSession(Serializable sessionId) {
        ...
        Session s = (Session) ShiroSerializeUtils.deserialize(redisManager.get(this.getByteKey(sessionId)));
        return s;
    }
